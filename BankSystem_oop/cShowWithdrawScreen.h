...
class cShowWithdrawScreen : protected cScreen
{
private:
	...
	static void _showWithdraw()
	{
		...
		if (cInput::askUser("\n" + cFormat::tab(2) + cFormat::color(("(*)")) + " Are you sure you want to confirm this operation"))
		{
			...
			{
				cout << endl << cFormat::tab(2) << cFormat::color("Withdraw Operation Successfully ^_^", cFormat::txGreen) << endl;
			...
			else
			{
				...
				cout << endl << cFormat::tab(2) << cFormat::color("Withdraw Operation Faild X_X, because you withdraw amount greater than your balance.") << endl;
				cout << endl << cFormat::tab(2) << "Amount: " << amount <<  cFormat::color(" $", cFormat::txGreen) << endl;
				cout << endl << cFormat::tab(2) << "Your Balance Is: " << cFormat::color(to_string(client.balance), cFormat::txBrightYellow) + cFormat::color(" $", cFormat::txGreen) << endl;
...
public:
	...
	{
		system("cls");
		printHeaderOfPage("Withdraw Screen");
		_showWithdraw();
	}
};
